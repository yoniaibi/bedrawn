...
const TYPES = [
  { id: 'single', label: 'Single item', emoji: '📦' },
    ...
    return (
      <AppShell>
        <div style={{ maxWidth: 500, margin: '0 auto', textAlign: 'center', padding: '60px 24px' }}>
          ...
          <p style={{ fontSize: 22, fontWeight: 700, color: 'var(--white)', margin: '0 0 8px' }}>Listing submitted!</p>
    ...
    <AppShell>
      <div style={{ maxWidth: 500, margin: '0 auto' }}>
        <div style={{ padding: '16px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <p style={{ margin: 0, fontSize: 18, fontWeight: 700, color: 'var(--white)' }}>List a new item</p>
        </div>
        ...
        <div style={{ padding: 16 }}>
          ...
          {step === 0 && (
            <div>
              <p style={{ margin: '0 0 16px', fontSize: 16, fontWeight: 700, color: 'var(--white)' }}>What type of draw?</p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {TYPES.map(t => (
                  <button key={t.id} onClick={() => setType(t.id)} style={{
                    padding: '20px 16px', borderRadius: 12, cursor: 'pointer', textAlign: 'center',
                    background: type === t.id ? 'rgba(139,92,246,0.15)' : 'var(--card)',
                    border: `2px solid ${type === t.id ? 'var(--purple)' : 'var(--border)'}`,
                  }}>
                    <p style={{ fontSize: 28, margin: '0 0 6px' }}>{t.emoji}</p>
                    <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: type === t.id ? 'var(--purple)' : 'var(--white)' }}>{t.label}</p>
                  </button>
                ))}
              </div>
            </div>
          ...
          {step === 1 && (
            <div>
              <p style={{ margin: '0 0 8px', fontSize: 16, fontWeight: 700, color: 'var(--white)' }}>Add photos</p>
              <p style={{ margin: '0 0 16px', fontSize: 13, color: 'var(--grey)' }}>Up to 6 photos. First photo is your hero image.</p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                {Array.from({ length: 6 }, (_, i) => (
                  <div key={i} style={{
                    aspectRatio: '1', borderRadius: 10,
                    background: i === 0 ? 'rgba(139,92,246,0.1)' : 'var(--card)',
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  {CONDITIONS.map(c => (
                    <button key={c} onClick={() => setCondition(c)} style={{
                      ...
                      background: condition === c ? 'rgba(139,92,246,0.15)' : 'var(--card)',
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  {CATEGORIES.map(c => (
                    <button key={c} onClick={() => setCategory(c)} style={{
                      ...
                      background: category === c ? 'rgba(139,92,246,0.15)' : 'var(--card)',
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  {STYLES.map(s => (
                    <button key={s} onClick={() => setStyle(s)} style={{
                      flex: 1, padding: '8px 0', borderRadius: 8, cursor: 'pointer', fontSize: 12,
                      background: style === s ? 'rgba(139,92,246,0.15)' : 'var(--card)',
          ...
          {step === 3 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {TICKET_PRICES.map(p => (
                    <button key={p} onClick={() => setTicketPrice(p)} style={{
                      ...
                      background: ticketPrice === p ? 'rgba(139,92,246,0.15)' : 'var(--card)',
              ...
              {earnings && (
                <div style={{ background: 'rgba(16,185,129,0.1)', border: '1px solid var(--green)', borderRadius: 12, padding: '14px 16px' }}>
                  <p style={{ margin: 0, fontSize: 14, color: 'var(--white)' }}>
                    At full sell-out you earn{' '}
                    <strong style={{ color: 'var(--green)', fontSize: 16 }}>£{earnings.toFixed(2)}</strong>
                  </p>
                  <p style={{ margin: '4px 0 0', fontSize: 12, color: 'var(--grey)' }}>77% of total ticket revenue</p>
                </div>
              )}
            </div>
          )}
          ...
          {step === 4 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <p style={{ margin: 0, fontSize: 16, fontWeight: 700, color: 'var(--white)' }}>Review your listing</p>
              <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 14, padding: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
                {[
                  ...
                  <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <span style={{ fontSize: 13, color: 'var(--white)', fontWeight: 600 }}>{row.value}</span>
                  </div>
                ))}
              </div>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
              {step > 0 && (
                <button onClick={() => setStep(s => s - 1)} style={{
                  ...
                  border: '1px solid var(--border)', color: 'var(--white)', fontWeight: 600, cursor: 'pointer',
                }}>Back</button>
              )}
              <button onClick={() => setStep(s => s + 1)} style={{
